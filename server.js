...
app.get('/*', async (req, res) => {
    ...
    fs.readFile(templateFile, 'utf8', (err, data) => {
        ...
        return res.send(
            data.replace('<div id="root"></div>', `<script>window.preloadedArticles = ${JSON.stringify(contextObj)};</script><div id="root">${reactApp}</div>`).replace('{{ styles }}', sheet.getStyleTags()));
    })
});
